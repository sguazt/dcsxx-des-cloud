/**
 * \file dcs/des/cloud/best_fit_decreasing_incremental_placement_strategy.hpp
 *
 * \brief Incremental VM placement based on the BEST-FIT DECREASING strategy.
 *
...
#ifndef DCS_DES_CLOUD_BEST_FIT_DECREASING_INCREMENTAL_PLACEMENT_STRATEGY_HPP
#define DCS_DES_CLOUD_BEST_FIT_DECREASING_INCREMENTAL_PLACEMENT_STRATEGY_HPP
		...
		for (vm_iterator vm_it = sorted_vms.begin(); vm_it != vm_end_it; ++vm_it)
		{
			...
			for (pm_iterator pm_it = sorted_pms.begin(); pm_it != pm_end_it && !placed; ++pm_it)
			{
				...
				for (share_iterator ref_share_it = ref_shares.begin(); ref_share_it != ref_share_end_it; ++ref_share_it)
				{
					...
					real_type share;
					share = ::dcs::des::cloud::scale_resource_share(ref_capacity,
															   //ref_threshold,
															   actual_capacity,
															   //actual_threshold,
															   ref_share);
...
#endif // DCS_DES_CLOUD_BEST_FIT_DECREASING_INCREMENTAL_PLACEMENT_STRATEGY_HPP
