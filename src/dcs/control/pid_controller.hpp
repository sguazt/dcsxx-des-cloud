...
{
	public: typedef RealType real_type;
...
pid_controller<RealType>::pid_controller(RealType kp, RealType ki, RealType kd, RealType step_time, RealType err_thresh)
	: kp_(kp),
	  ki_(ki),
	  kd_(kd),
	  h_(step_time),
...
template <typename RealType>
RealType pid_controller<RealType>::update(RealType error)
{
	// Update if the error magnitude is below the threshold.
	if (
			err_thresh_ == std::numeric_limits<RealType>::infinity()
...
template <typename RealType>
std_pid_controller<RealType>::std_pid_controller(RealType kp, RealType int_time, RealType deriv_time, RealType step_time, RealType err_thresh)
...
template <typename RealType>
RealType std_pid_controller<RealType>::update(RealType error)
{
